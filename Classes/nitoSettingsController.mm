...
-(id)initWithTitle:(NSString *)theTitle
{
	...
	if (self != nil)
	{
		...
		BRImage *settingsImage = [[BRThemeInfo sharedTheme] gearImage];
...
- (id)previewControlForItem:(long)item
{
	...
	switch (item) {
		...
		case kNUpdateNitoTVMenuItem: //update nitoTV
				...
				//previewImage = [THEME applianceIcon];
				//break;
		...
		case kNUpdateCheckFrequencyMenuItem:
			...
			//previewImage = [THEME applianceIcon];
			previewImage = [[BRThemeInfo sharedTheme] gearImage];
			break;
...
+ (int)updateStatus //0 = update possible (block menu option) 1 = update blocked
{
...
+ (int)notificationStatus //0 = hooks off 1 = hooks on
{
...
+ (NSString *)stringForTime:(int)checkTime
{
	switch (checkTime) {
...
-(id)itemForRow:(long)row
{
	...
	switch (row) {
		...
		case kNUpdateNitoTVMenuItem: // install nitoTV
			...
			break;
		/*
		case kNToggleUpdateMenuItem: //block/unblock
			result = [BRMenuItem ntvMenuItem];
			
			status = [nitoSettingsController updateStatus];
			switch (status) {
					
				case 0:
					
					[result setRightJustifiedText: BRLocalizedString(@"Enabled", @"Enabled") withAttributes:[[BRThemeInfo sharedTheme] menuItemSmallTextAttributes]];
					
					break;
					
				case 1:
					
					[result setRightJustifiedText: BRLocalizedString(@"Disabled", @"Disabled") withAttributes:[[BRThemeInfo sharedTheme] menuItemSmallTextAttributes]];
...
//			result = [BRMenuItem ntvMenuItem];
//			
//			status = [nitoSettingsController notificationStatus];
//			switch (status) {
//					
//				case 0:
//					
...
//			break;
		...
		case kNUpdateCheckFrequencyMenuItem:
			...
			status = [nitoSettingsController updateCheckFrequency];
			[result setRightJustifiedText:[nitoSettingsController stringForTime:status]	withAttributes:[[BRThemeInfo sharedTheme] menuItemSmallTextAttributes]];
			
			break;
			
		case kNViewSyslogMenuItem: //system log
			result = [BRMenuItem ntvComputerMenuItem];
			break;
		case kNRestartLowtideMenuItem: //restart Lowtide
		case kNRebootAppleTVMenuItem:	//reboot AppleTV
			result = [BRMenuItem ntvRefreshMenuItem];
			break;
...
- (BRAlertController *)_alertWithMessage:(NSString *)errorDescription andTitle:(NSString *)theTitle
{
	
	NSString *errorString = [NSString stringWithFormat:BRLocalizedString(@"%@", @"primaryText for generic settings alert"), errorDescription];
	
	BRAlertController *result = [[BRAlertController alloc] initWithType:0 titled:theTitle primaryText:errorString secondaryText:BRLocalizedString(@"Re-directed to settings facade in 5 seconds", @"secondary text when error in settings controller")];
...
{
	BRListControl *theList = [[scrollBarInfo userInfo] valueForKey:@"ListControl"];
	int theCount = [theList dataCount];
...
- (void)itemSelected:(long)selected
{
	...
	BRScrollingTextControl *textControls = nil;
	...
	switch (selected) {
		...
		case kNUpdateNitoTVMenuItem: //update nitoTV
			...
			spinControl = [[BRTextWithSpinnerController alloc] initWithTitle:BRLocalizedString(@"Updating, Please wait...", @"title for Spinner text control while updating nitoTV") text:BRLocalizedString(@"Updating nitoTV, please wait...",@"main text for Spinner text control while updating nitoTV" )];
			...
			/*
		case kNToggleUpdateMenuItem: //toggle update
			
			returnValue = (BOOL)[self toggleUpdate];
			if(returnValue == YES)
			{
				
				controller = [self _alertWithMessage:BRLocalizedString(@"AppleTV OS updates successfully disabled!",@"AppleTV OS updates successfully disabled!" ) andTitle:BRLocalizedString(@"Update Disabled",@"Update Disabled") ];
			} else if (returnValue == NO)
			{
					//NSLog(@"show dialog saying update was unblocked");
				controller = [self _alertWithMessage:BRLocalizedString(@"AppleTV OS updates successfully enabled!", @"AppleTV OS updates successfully enabled!") andTitle:BRLocalizedString(@"Update Enabled", @"Update Enabled")];
			}
			[[self stack] pushController:controller];
			[controller release];
			
			[NSTimer scheduledTimerWithTimeInterval: 5 target: self 	selector: @selector(popTop) userInfo: nil repeats: NO];
			[[self list] reload];
			break;
		...
		case kNViewSyslogMenuItem: //view system log
			...
			textControls = [[BRScrollingTextControl alloc] init];
			scrollBar = [[[[textControls textBox] list] providers] objectAtIndex:0];
				//NSLog(@"list %@", scrollBar);
			[textControls setDocumentPath:@"/tmp/temp.log" encoding:NSUTF8StringEncoding];
			[textControls setTitle:@"syslog"];
			
			controller =  [BRController controllerWithContentControl:textControls];
			...
			lcd = [NSDictionary dictionaryWithObject:[[textControls textBox] list] forKey:@"ListControl"];
...
{
	int currentState = [nitoSettingsController notificationStatus];
	switch (currentState) {
...
{
	int currentState = [nitoSettingsController updateCheckFrequency];
	switch (currentState) {
...
//- (void)blockUpdate
//{
//	NSString *command = @"/usr/libexec/nitotv/setuid /usr/libexec/nitotv/blockUpdate.sh";
//	int sysReturn = system([command UTF8String]);
//	id alertController = nil;
//	if (sysReturn == 0)
//	{
//		alertController = [self blockSuccess];
//		
//	} else {
//		
//		alertController = [self blockFailed];
//	}
//	
//	[[self stack] pushController:alertController];
//	//NSLog(@"nothing for now");
//}
...
-(id)blockSuccess
{
	BRAlertController *alertCon = [BRAlertController alertOfType:0 titled:BRLocalizedString(@"Updates Blocked Successfully", @"update blocker alert title for success") primaryText:BRLocalizedString(@"Software Update Blocked Successfully!",@"update blocker alert main text for success" ) secondaryText:BRLocalizedString(@"press menu to exit", @"press menu to exit")];
	return alertCon;
}

-(id)blockFailed
{
	BRAlertController *alertCon = [BRAlertController alertOfType:0 titled:BRLocalizedString(@"Update Block Failed", ) primaryText:BRLocalizedString(@"Software Updates Blocking Failed!", ) secondaryText:BRLocalizedString(@"press menu to exit",@"press menu to exit") ];
	return alertCon;
}
