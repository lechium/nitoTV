...
{
    [_meta setObject:title forKey:NTV_META_TITLE];
}
-(void)setSummary:(NSString *)summary
{
    [_meta setObject:summary forKey:NTV_META_SUMMARY];
}
-(void)setCustomKeys:(NSArray *)keys forObjects:(NSArray *)objects
{
    if([keys count]==[objects count])
    {
        [_meta setObject:keys forKey:NTV_META_CUSTOM_KEYS];
        [_meta setObject:objects forKey:NTV_META_CUSTOM_OBJECTS];
    }
}
-(BRImage *)coverArt
{
...
-(NSDictionary *)orderedDictionary
{
    ...
    if([_meta objectForKey:NTV_META_TITLE]!=nil)
        [a setObject:[_meta objectForKey:NTV_META_TITLE] forKey:NTV_META_TITLE];
    if([_meta objectForKey:NTV_META_SUMMARY]!=nil)
        [a setObject:[_meta objectForKey:NTV_META_SUMMARY] forKey:NTV_META_SUMMARY];
    if ([_meta objectForKey:NTV_META_CUSTOM_KEYS]!=nil && [_meta objectForKey:NTV_META_CUSTOM_OBJECTS]!=nil) {
        [a setObject:[_meta objectForKey:NTV_META_CUSTOM_KEYS] forKey:NTV_META_CUSTOM_KEYS];
        [a setObject:[_meta objectForKey:NTV_META_CUSTOM_OBJECTS] forKey:NTV_META_CUSTOM_OBJECTS];
...
-(id)title
{
    return [_meta objectForKey:NTV_META_TITLE];
}
-(id)summary
{
    return [_meta objectForKey:NTV_META_SUMMARY];
}
- (BOOL)hasCoverArt
{
	return YES;
