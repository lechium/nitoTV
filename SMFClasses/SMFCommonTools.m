...
//        size=CGSizeMake(0.9,0.15);
	...
	Class brpm = objc_getClass("BRPopUpManager");
	
    id manager  = [brpm sharedInstance];
    if (manager==nil) {
        [brpm setSingleton:[[[brpm alloc]init]autorelease]];
    }
    [[brpm sharedInstance] postPopUpWithControl:popup 
                                               identifier:@"SMFPopup" 
...
-(void)restartLowtide
{
    [[objc_getClass("BRApplication") sharedApplication]terminate];
}
@end
